...
export const normalizeConfigResponse = (raw: unknown): Config => {
  ...
  const requestRetryValue = normalizeNumber(requestRetry);
  if (requestRetryValue !== undefined) {
    config.requestRetry = requestRetryValue;
  }
  const maxRetryCredentialsValue = normalizeNumber(
    raw['max-retry-credentials'] ?? raw.maxRetryCredentials
  );
  if (maxRetryCredentialsValue !== undefined) {
    config.maxRetryCredentials = maxRetryCredentialsValue;
  }
  const maxRetryIntervalValue = normalizeNumber(raw['max-retry-interval'] ?? raw.maxRetryInterval);
  if (maxRetryIntervalValue !== undefined) {
    config.maxRetryInterval = maxRetryIntervalValue;
  }
  const rpmLimitDefaultValue = normalizeNumber(raw['rpm-limit-default'] ?? raw.rpmLimitDefault);
  if (rpmLimitDefaultValue !== undefined) {
    config.rpmLimitDefault = rpmLimitDefaultValue;
  }
  const tpmLimitDefaultValue = normalizeNumber(raw['tpm-limit-default'] ?? raw.tpmLimitDefault);
  if (tpmLimitDefaultValue !== undefined) {
    config.tpmLimitDefault = tpmLimitDefaultValue;
  }
  const concurrencyLimitDefaultValue = normalizeNumber(
    raw['concurrency-limit-default'] ?? raw.concurrencyLimitDefault
  );
  if (concurrencyLimitDefaultValue !== undefined) {
    config.concurrencyLimitDefault = concurrencyLimitDefaultValue;
  }
  const rphLimitDefaultValue = normalizeNumber(raw['rph-limit-default'] ?? raw.rphLimitDefault);
